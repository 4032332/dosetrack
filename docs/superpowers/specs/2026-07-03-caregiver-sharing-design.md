...
CoreData on the patient's own device remains the on-device source of truth, exactly as today — this feature does not migrate patients to a server-source-of-truth model. `SupabaseSyncManager` (existing) already pushes local `Medication`/`Schedule`/`DoseLog` writes to Supabase and pulls/merges remote rows back via `pullAll`/`mergeMedications`/`mergeSchedules`/`mergeDoseLogs`. Today these functions have **no user-scoping parameter** — they read `AuthManager.shared.session?.user.id` internally and rely entirely on RLS filtering the currently-authenticated session's own rows. This feature extends that mechanism but requires real (not cosmetic) changes to it:

- When a caregiver relationship is `active`, the patient's own device continues syncing exactly as it does now, unchanged — the RLS change described above only affects who else is *authorized* to read/write those Supabase rows.
- **New requirement, not a reuse-as-is:** `pullAll`, `pushMedication`, `pushSchedule`, and `pushDoseLog` must be changed to accept an explicit target `userId` parameter (rather than implicitly reading the current session's own id), and all their underlying queries/upserts must filter/tag rows by that parameter instead of assuming "the signed-in user." This is a genuine code change to `SupabaseSyncManager`'s function signatures and query logic, scoped as part of this feature's implementation — not something that falls out for free from existing RLS.
- The caregiver's device, when its `ActiveAccountContext` is set to a patient, calls these updated functions with the patient's `user_id`, writing into a **separate local CoreData store scoped to that patient** (not merged into the caregiver's own medications). Supabase's RLS policy (not a client-side check) is what actually prevents a caregiver from passing an arbitrary `userId` they're not authorized for — the client-side parameter only selects *which* authorized account's data to fetch/write.
...
- Net effect: this reuses the existing sync *architecture* (push/pull/merge against Supabase tables) rather than building a parallel sync system, but it is not a drop-in reuse — `SupabaseSyncManager`'s functions need to become user-parameterized as part of this implementation.
