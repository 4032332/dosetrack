...
- **Cross-frame accumulation (`ScanTextAccumulator`):** recognised text is accumulated across ALL frames of a session (deduped, keeping the longest variant and greatest height per line), and the parser runs against the *whole accumulation*, not just the current frame. This is what lets it read a **cylindrical bottle**: the wrapped label is never fully visible in one frame, so the user rotates the bottle and each field is captured once and retained instead of lost when it scrolls off. Also makes flat boxes more forgiving. A "Rescan" button clears the accumulation for aiming at a different medication.
